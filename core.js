...
// Menutup Excel Filter jika user men-scroll tabel secara horizontal/vertikal
        document.addEventListener('scroll', function(e) {
            if(e.target && e.target.classList && e.target.classList.contains('table-responsive')) {
                let excelFilter = document.getElementById('excel-filter-modal');
                if (excelFilter && excelFilter.style.display === 'block') {
...
document.getElementById('tbody-line-items').addEventListener('scroll', function() {
        ...
        document.getElementById('tbody-line-items').addEventListener('input', function(e) {
            if(e.target.classList.contains('line-gl')) {
                ...
                // Filter distinct GLs based on input
                let allGLs = [...new Set(masterGL.map(g => g.gl))];
        ...
        document.addEventListener('keydown', function(e) {
            ...
            if (e.key === 'Escape') {
                ...
                let efModal = document.getElementById('excel-filter-modal');
        ...
        window.applyClaimPeriodPreset = function(module, preset) {
            ...
            if(picker) {
                if(range) picker.setDate(window[config.targetKey], false);
    ...
    customConfirm('Apakah Anda yakin ingin keluar dari sistem?', async () => {
        await logActivity(sessionUser, 'Keluar dari Aplikasi');
...
function renderActivityLogs() {
    ...
    if (!Array.isArray(activityLogs) || activityLogs.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" style="text-align:center; color:#888;">Belum ada log aktivitas.</td></tr>';
    ...
    if(!pageRows.length) {
        tbody.innerHTML = '<tr><td colspan="7" style="text-align:center; color:#888;">Tidak ada log yang cocok dengan filter.</td></tr>';
    }

    pageRows.forEach(log => {
        const meta = getActivityLogMeta(log);
        const eventId = escapeActivityLogText(meta.eventId);
        const claimHint = meta.claimId ? `<br><small style="color:#6b7f91;">Klaim ${escapeActivityLogText(meta.claimNo !== '-' ? meta.claimNo : meta.claimId)}${meta.claim ? '' : ' · data tidak tersedia'}</small>` : '';
        tbody.innerHTML += `<tr>
            ...
            <td><button type="button" class="btn btn-info" data-event-id="${eventId}" onclick="openActivityLogDetail(this.dataset.eventId)" style="font-size:10px; padding:4px 8px;">Detail</button></td>
        </tr>`;
    });
...
function sortDataArray(arr, order) {
    return arr.sort((a, b) => {
        ...
        function updateClock() {
            ...
            if(clockEl) clockEl.innerText = now.toLocaleString('id-ID', { weekday:'short', year:'numeric', month:'short', day:'numeric', hour:'2-digit', minute:'2-digit', second:'2-digit'});
        }
...
window.openExcelFilter = function(e, colKey, module) {
    ...
    listContainer.innerHTML += `<label class="ef-cb-item" style="font-weight:bold; border-bottom:1px solid #ccc; padding-bottom:5px; margin-bottom:5px;">
    ...
    if (useTree) {
        ...
        Object.keys(treeData).sort((a,b)=>b-a).forEach(y => {
            treeHtml += `<div class="tree-group-y" style="margin-bottom:4px;">
                <div style="display:flex; align-items:center; gap:5px; font-weight:bold; background:#f0f4f8; padding:3px 5px; border-radius:4px;">
                    <span style="cursor:pointer; width:12px; text-align:center; font-family:monospace; color:#0050A0;" onclick="let n = this.parentElement.nextElementSibling; n.style.display = n.style.display === 'none' ? 'block' : 'none'; this.innerText = n.style.display === 'none' ? '+' : '-';">-</span>
                    <label class="ef-cb-item" style="flex:1; margin:0;"><input type="checkbox" class="cb-tree cb-y" onchange="toggleTreeCb(this, '.cb-m, .ef-cb-val')"> <span>Tahun ${y}</span></label>
                </div>
                <div class="tree-child-y" style="padding-left:15px; display:block; margin-top:2px;">`;
            
            Object.keys(treeData[y]).sort((a,b)=>a-b).forEach(m => {
                treeHtml += `<div class="tree-group-m" style="margin-bottom:2px;">
                    <div style="display:flex; align-items:center; gap:5px; background:#fafbfc; padding:2px 5px; border-radius:4px; border-left:2px solid #0050A0;">
                        <span style="cursor:pointer; width:12px; text-align:center; font-family:monospace; color:#0050A0;" onclick="let n = this.parentElement.nextElementSibling; n.style.display = n.style.display === 'none' ? 'block' : 'none'; this.innerText = n.style.display === 'none' ? '+' : '-';">-</span>
                        <label class="ef-cb-item" style="flex:1; margin:0;"><input type="checkbox" class="cb-tree cb-m" onchange="toggleTreeCb(this, '.ef-cb-val')"> <span style="font-weight:600;">${monthNames[m] || m}</span></label>
                    </div>
                    <div class="tree-child-m" style="padding-left:15px; display:block; margin-top:2px;">`;
                
                treeData[y][m].forEach(val => {
                    let d = val.split('/')[0];
                    let isChecked = isAllChecked || activeFilters.includes(val.toLowerCase());
                    let safeVal = val.replace(/"/g, '&quot;');
                    treeHtml += `<label class="ef-cb-item ef-data-item" style="padding:2px 5px;"><input type="checkbox" class="ef-cb-val cb-d" value="${safeVal}" ${isChecked ? 'checked' : ''} onchange="checkExcelFilterIndividually(); syncAllTreeCb();"> <span>${parseInt(d, 10)}</span></label>`;
                });
        ...
        emptyVals.forEach(val => {
            let isChecked = isAllChecked || activeFilters.includes(val.toLowerCase());
            let safeVal = val.replace(/"/g, '&quot;');
            treeHtml += `<label class="ef-cb-item ef-data-item"><input type="checkbox" class="ef-cb-val" value="${safeVal}" ${isChecked ? 'checked' : ''} onchange="checkExcelFilterIndividually()"> <span>${val}</span></label>`;
        });
        ...
        efUniqueValues.forEach(val => {
            let isChecked = isAllChecked || activeFilters.includes(val.toLowerCase());
            let safeVal = val.replace(/"/g, '&quot;');
            listContainer.innerHTML += `<label class="ef-cb-item ef-data-item"><input type="checkbox" class="ef-cb-val" value="${safeVal}" ${isChecked ? 'checked' : ''} onchange="checkExcelFilterIndividually()"> <span>${val}</span></label>`;
        });
    }
...
window.checkExcelFilterIndividually = function() {
...
window.syncAllTreeCb = function() {
    document.querySelectorAll('.tree-group-y').forEach(yGroup => {
        let mGroups = yGroup.querySelectorAll('.tree-group-m');
        let allYChecked = true; let yVisible = false;

        mGroups.forEach(mGroup => {
            let dCbs = Array.from(mGroup.querySelectorAll('.cb-d'));
            let visibleD = dCbs.filter(cb => cb.closest('.ef-data-item') && cb.closest('.ef-data-item').style.display !== 'none');
            
            if (visibleD.length > 0) {
                yVisible = true; mGroup.style.display = 'block';
                let checkedD = visibleD.filter(cb => cb.checked).length;
                let mCb = mGroup.querySelector('.cb-m');
                if(mCb) mCb.checked = (checkedD === visibleD.length);
        ...
        let yCb = yGroup.querySelector('.cb-y');
        if(yCb) yCb.checked = (yVisible && allYChecked && mGroups.length > 0);
        ...
        function applyExcelSort(dir) {
            if(efActiveModule && efActiveCol) {
                ...
                document.getElementById('excel-filter-modal').style.display = 'none';
    ...
    if(efActiveModule && efActiveCol) {
        let allCheckboxes = document.querySelectorAll('.ef-cb-val');
        let allChecked = document.querySelectorAll('.ef-cb-val:checked');

        // Jika semua tercentang -> Reset/Hapus Filter (Gak perlu cek lagi ada keyword atau nggak)
        if (allChecked.length === allCheckboxes.length && allCheckboxes.length > 0) {
            ...
            document.querySelectorAll('.ef-data-item').forEach(lbl => {
                ...
                // Ambil semua centangan tanpa peduli dia lagi ngumpet di layar atau nggak
                if (cb && cb.checked) {
                    selectedVals.push(cb.value.toLowerCase().trim());
                }
            });
            
            // Validasi: Kalau user klik Apply tapi gak ada satupun yang dicentang
            if (selectedVals.length === 0) {
                return showToast('Pilih minimal satu filter atau gunakan tombol Bersihkan.', 'error');
            }
            
            tableFilters[efActiveModule][efActiveCol] = selectedVals;
        }
...
function updateFilterIconHighlight(module) {
    ...
    let idPrefix = module === 'karyawan' ? '#menu-master-karyawan' : (module === 'history' ? '#menu-history' : (module === 'super-find' ? '#menu-super-find' : (module === 'waiting' ? '#menu-waiting-approval' : `#menu-claim-${module}`)));
    ...
    let menuId = module === 'karyawan' ? 'menu-master-karyawan' : (module === 'history' ? 'menu-history' : (module === 'super-find' ? '#menu-super-find' : (module === 'waiting' ? 'menu-waiting-approval' : 'menu-claim-' + module)));
    ...
    updateBackButtonVisibility();
        ...
        function saveDocModal() {
            if(currentDocTargetId) { 
                ...
                targetEl.value = compressed ? "👁️ Lihat Ref No Doc" : "📄 + Input Ref Doc";
            ...
            if(urgentData.length > 0) {
                ...
                urgentData.forEach(item => {
                    ...
                    tbody.innerHTML += `<tr>
                        ...
                        <td><strong>${item.nama}</strong><br><span style="font-size:11px;color:#777;">${item.nik}</span></td>
                        <td>${item.tipe}</td>
                        <td><strong style="color:#0050A0;">${formatClaimMoney(item)}</strong></td>
                        <td><span class="badge status-revise">${item.statusClaim}</span><br><span style="color:#dc3545; font-size:11px; font-weight:bold;">Terlambat ${diffDays} hari</span></td>
...
function toggleSelectAll(master, className) {
    ...
    const filtered = getFilteredRowsForSource(module);
...
let editKarIdx = null;
function autoFillKaryawan(nikVal) {
    let k = masterKaryawan.find(x => x.nik === nikVal.trim());
    let namaInp = document.getElementById('hdr-nama');
    let entSel = document.getElementById('hdr-entitas');
    if(k) {
        namaInp.value = k.nama;
        entSel.value = k.entitas || 'AIO';
        namaInp.readOnly = true; entSel.disabled = true;
        namaInp.style.backgroundColor = '#e9ecef'; entSel.style.backgroundColor = '#e9ecef';
        namaInp.style.color = '#666'; entSel.style.color = '#666';
    } else {
        namaInp.value = ''; entSel.value = 'AIO';
        if(nikVal.trim() === '0000') {
            namaInp.readOnly = false; entSel.disabled = false;
            namaInp.style.backgroundColor = '#fff'; entSel.style.backgroundColor = '#fff';
            namaInp.style.color = '#333'; entSel.style.color = '#333';
        } else {
            namaInp.readOnly = true; entSel.disabled = true;
            namaInp.style.backgroundColor = '#e9ecef'; entSel.style.backgroundColor = '#e9ecef';
            namaInp.style.color = '#666'; entSel.style.color = '#666';
...
function autoFillKaryawanQuick(nikVal) {
    let k = masterKaryawan.find(x => x.nik === nikVal.trim());
    let namaInp = document.getElementById('qk-nama');
    let entSel = document.getElementById('qk-entitas');
    if(k) {
        namaInp.value = k.nama;
        entSel.value = k.entitas || 'AIO';
        namaInp.readOnly = true; entSel.disabled = true;
...
function importKaryawanExcel(e) {
    ...
    r.readAsArrayBuffer(file);
        ...
        function renderMasterGL() {
            ...
            let tipes = [...new Set(masterGL.map(g => g.tipe))];
            if(!tipes.includes("Operasional")) tipes.push("Operasional");
            if(!tipes.includes("Traveling")) tipes.push("Traveling");
            if(!tipes.includes("Entertain")) tipes.push("Entertain");
            if(!tipes.includes("Others")) tipes.push("Others");

            let dl = document.getElementById('dl-tipe-pengajuan'); dl.innerHTML = '';
            tipes.forEach(t => dl.innerHTML += `<option value="${t}">`);

            let isAdmin = sessionRole === 'admin';
            document.querySelectorAll('.admin-col-gl').forEach(el => el.style.display = isAdmin ? '' : 'none');

            masterGL.forEach((gl, idx) => { 
                let actionCols = isAdmin ? `<td><input type="checkbox" class="gl-checkbox" data-idx="${idx}"></td><td><button class="btn-icon" onclick="editGL(${idx})" title="Edit">✏️</button></td>` : '';
                tbody.innerHTML += `<tr>${actionCols}<td><span class="badge status-revise">${gl.tipe}</span></td><td>${gl.gl}</td></tr>`; 
            });
        ...
        function saveGLMaster() {
            if(!requireAdmin()) return;
            let tipe = document.getElementById('new-gl-tipe').value.trim(); 
            if(!tipe) tipe = "Others";
            let gl = document.getElementById('new-gl-nama').value.trim();
            if(!gl) return;

            if (/[0-9]/.test(gl)) {
                return showToast('Akun GL tidak boleh mengandung angka.', 'error');
            }

            if(editGLIdx !== null) {
        if(masterGL.find((g, i) => g.tipe.toLowerCase()===tipe.toLowerCase() && g.gl.toLowerCase()===gl.toLowerCase() && i !== editGLIdx)) return showToast('Data GL tersebut telah tersedia.', 'error');
        masterGL[editGLIdx] = {tipe: toTitleCase(tipe), gl}; editGLIdx = null; document.getElementById('btn-save-gl').innerText = '+ Tambah'; document.getElementById('btn-save-gl').classList.replace('btn-success', 'btn-primary');
    } else {
        if(masterGL.find(g => g.tipe.toLowerCase()===tipe.toLowerCase() && g.gl.toLowerCase()===gl.toLowerCase())) return showToast('Data GL tersebut telah tersedia.', 'error');
        masterGL.push({tipe: toTitleCase(tipe), gl});
    }
    renderMasterGL(); 
    document.getElementById('new-gl-nama').value=''; 
    showToast('Data GL berhasil disimpan.', 'success');
    logActivity(sessionUser, `Pembaruan Data Induk Akun GL: ${gl}`);
    
    // PANGGIL INI: Biar kesimpan di IndexedDB lokal & memicu refresh info storage sidebar!
    saveDataToLocal({ claimIds: [] }); 
}
        
        function editGL(idx) {
            let g = masterGL[idx]; document.getElementById('new-gl-tipe').value = g.tipe; document.getElementById('new-gl-nama').value = g.gl;
            editGLIdx = idx; document.getElementById('btn-save-gl').innerText = '✔️ Update'; document.getElementById('btn-save-gl').classList.replace('btn-primary', 'btn-success');
        }
    ...
    if(checked.length) { 
        ...
        function importGLExcel(e) {
            ...
            r.onload = function(evt) {
                ...
                json.forEach(row => {
                    if(row.length >= 2) {
                        let tRaw = row[0]?row[0].toString().trim():"Others"; let gl = row[1]?row[1].toString().trim():""; 
                        let tTarg = toTitleCase(tRaw);
                        if(tTarg && gl && gl.toLowerCase()!=="gl account") { 
                            if(!masterGL.find(g=>g.tipe.toLowerCase()===tTarg.toLowerCase() && g.gl.toLowerCase()===gl.toLowerCase())) { 
                                masterGL.push({tipe: tTarg, gl}); c++; 
                            } 
                        }
                    }
                }); 
                renderMasterGL(); showToast(`${c} data GL berhasil diimpor.`, 'success'); e.target.value = '';
                logActivity(sessionUser, `Impor ${c} Data Induk GL dari Excel`);
            }; r.readAsArrayBuffer(file);
        }
...
document.addEventListener('click', function(e) {
    ...
    if (filterModal && filterModal.style.display === 'block') {
        if (!filterModal.contains(e.target) && !e.target.classList.contains('th-filter-icon')) {
...
const WORKSHEET_TRANSLATION_ROWS = [
    ...
    ['Penyimpanan Lokal Utama · Sinkronisasi Latar Belakang · Pembacaan Delta', 'Local-first Storage · Background Sync · Delta Reads', 'ローカル優先保存・バックグラウンド同期・差分読込'],
    ...
    ['Nomor PR', 'PR Number', 'PR番号'],
    ['No. Ref', 'Reference No.', '参照番号'],
...
const WORKSHEET_ADDITIONAL_TRANSLATION_ROWS = [
    ...
    ['Form Input Cepat (Tanpa Detail Baris)', 'Quick Entry Form (Without Detail Rows)', 'クイック入力フォーム（明細行なし）'],
    ...
    ['Finance', 'Finance', '財務'],
    ['Finance (Alur Pembayaran)', 'Finance (Payment Workflow)', '財務（支払フロー）'],
    ['Viewer', 'Viewer', '閲覧者'],
    ...
    ['Cari user, aktivitas, No. Ref, atau ID claim...', 'Search user, activity, reference number, or claim ID...', 'ユーザー、操作、参照番号、申請IDを検索...'],
